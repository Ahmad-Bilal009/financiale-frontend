...
router.get('/visitor', (req, res) => {
  ...
  const queries = {
    ...
    visitorsToday:
      'SELECT COUNT(*) AS total FROM visitors WHERE visit_date = CURDATE()',
    visitorsThisWeek:
      'SELECT COUNT(*) AS total FROM visitors WHERE YEARWEEK(visit_date, 1) = YEARWEEK(CURDATE(), 1)'
  }
  ...
  Object.entries(queries).forEach(([key, query]) => {
    db.query(query, (err, results) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: `Database query error for ${key}` })
      }
