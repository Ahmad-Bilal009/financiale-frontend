...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (!title || !description || !productType) {
      return res.status(400).json({ message: "Title, description, and product type are required" });
    }
    ...
    const product = await Product.create({
      ...
      geographicCoverage: Array.isArray(geographicCoverage) ? geographicCoverage : [],
...
// ✅ Get all products (Optionally filter approved products)
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    await product.update({
      ...
      stageOfEntrepreneurship: stageOfEntrepreneurship || product.stageOfEntrepreneurship,
      objectiveOfCredit: objectiveOfCredit || product.objectiveOfCredit,
      benefitsOfEntrepreneurship: benefitsOfEntrepreneurship || product.benefitsOfEntrepreneurship,
      geographicCoverage: Array.isArray(geographicCoverage) ? geographicCoverage : product.geographicCoverage,
      requirement: requirement || product.requirement,
      contactDetail: contactDetail ? JSON.stringify(contactDetail) : product.contactDetail,
    });
